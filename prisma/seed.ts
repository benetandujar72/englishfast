...
// FCE Core Vocabulary - Starter Set (200 words from key categories)
...
async function main() {
  ...
  // The seed is user-independent; vocabulary gets assigned when user is created
  // For now, just verify the data structure is valid
  console.log(`Prepared ${FCE_VOCABULARY.length} vocabulary items`);

  // If a user ID is provided, seed for that user
  const userId = process.argv[2];
  if (userId) {
    console.log(`Seeding vocabulary for user: ${userId}`);

    for (const word of FCE_VOCABULARY) {
    ...
    console.log(`Seeded ${FCE_VOCABULARY.length} words for user ${userId}`);
  } else {
    console.log("No user ID provided. Run with: npx prisma db seed -- <userId>");
    console.log("Vocabulary will be seeded on first user login.");
  }
}
